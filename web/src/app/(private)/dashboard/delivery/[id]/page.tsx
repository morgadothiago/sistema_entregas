...
export default function DeliveryDetailPage() {
  ...
  const deliveryCode = params?.id

  useEffect(() => {
    console.log("Params completos:", params)
    console.log("Código do delivery extraído:", deliveryCode)
    ...
    const fetchDelivery = async () => {
      try {
        if (!deliveryCode) {
          console.error("Código do delivery não encontrado")
          throw new Error("Código do delivery não encontrado")
        }

        console.log("Fazendo requisição para delivery código:", deliveryCode)
        console.log(
          "URL completa:",
          `${process.env.NEXT_PUBLIC_NEXTAUTH_API_HOST}/gps/delivery/${deliveryCode}`
        )

        // Limpar o código removendo possíveis caracteres problemáticos
        const cleanCode = deliveryCode.toString().split(":")[0] // Remove tudo após ':'
        console.log("Código limpo:", cleanCode)

        const response = await api.getDeliveryByCode(cleanCode, token)
        console.log("Resposta da API:", response)

        if ("status" in response) {
          const errorMessage = (response as any).message || "Erro desconhecido"
          console.error("Erro da API:", response)
          toast.error("Erro ao buscar delivery", {
        ...
        setDeliveryDetail(response as DeliveryUser)
      } catch (error: unknown) {
