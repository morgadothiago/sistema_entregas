...
export function normalizeData(userInfo: any, accessData: any) {
  ...
  if (!validateField(userInfo.vehicleType)) missingFields.push('Tipo de veículo');
  ...
  try {
    if (userInfo.dob instanceof Date) {
      ...
      if (userInfo.dob.year && userInfo.dob.month && userInfo.dob.day) {
        formattedDob = `${userInfo.dob.year}-${String(userInfo.dob.month).padStart(
          2,
          "0"
  ...
  const formattedPhone = userInfo.phone ? userInfo.phone.replace(/[^\d]/g, '') : '';
  
  // Formatar vehicleType (garantir que seja uma string)
  const vehicleType = typeof userInfo.vehicleType === 'object' && userInfo.vehicleType?.value 
    ? userInfo.vehicleType.value 
    : (typeof userInfo.vehicleType === 'string' ? userInfo.vehicleType : '');
